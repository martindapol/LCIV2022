...
public class ControladorOrden {
    ...
    public ControladorOrden() {
        conexion = "jdbc:mysql://localhost/mydb?user=root&password=root9718";
    ...
    public boolean registrarOrdenProduccion(OrdenProduccion orden){
        ...
        try( Connection cnn =  DriverManager.getConnection(conexion); 
                PreparedStatement ps = cnn.prepareStatement("INSERT INTO PRODUCCION (fecha, idResponsable, idComponente, cantidadProduccion) VALUES(?, ?, ?, ?))");
                ) {
    ...
    public List<OrdenProduccion> obtenerOrdenesResponsable(int id) {
        ...
        try( Connection cnn =  DriverManager.getConnection(conexion); 
                Statement stmt = cnn.createStatement();
                ResultSet rs = stmt.executeQuery("SELECT t.*, t1.nombre as n_resp, t2.nombre n_comp "
                        + " FROM PRODUCCION t, RESPONSABLE t1, COMPONENTE t2 WHERE "
                        + " t.idresponsable = t1.idresponsable"
                        + " AND t.idcomponente = t2.idcomponente "
                        + " AND t.id = " + id)) {
